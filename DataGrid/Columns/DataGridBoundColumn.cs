    ...
    public abstract class DataGridBoundColumn : DataGridColumn
    {
        ...
        public virtual Binding Binding
        {
            ...
            set
            {
                if (_binding != value)
                {
                    if (this.OwningGrid != null && !this.OwningGrid.CommitEdit(true /*exitEditing*/))
                    {
                        // Edited value couldn't be committed, so we force a CancelEdit
                        this.OwningGrid.CancelEdit(false /*raiseEvents*/);
                    }
        ...
        {
            if (this.OwningGrid != null && this.OwningGrid.ItemDataType != null &&
                this.Header == null && this.Binding != null && this.Binding.Path != null)
            {
                string header = this.OwningGrid.ItemDataType.GetDisplayName(this.Binding.Path.Path);
                if (header != null)
                {
                    this.Header = header;
                }
